...
-- constants
-- --------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `brimaslead` DEFAULT CHARACTER SET utf8 ;
USE `brimaslead` ;
...
CREATE TABLE IF NOT EXISTS `branch` (
  ...
  `fk_the_company_details_id_branch` INT(11) NULL DEFAULT NULL,
   `created_at` TIMESTAMP,
  `update_at` TIMESTAMP,
...
CREATE INDEX `fk_the_company_details_id_branches_idx` ON `branch` (`fk_the_company_details_id_branch` ASC) VISIBLE;
...
CREATE TABLE IF NOT EXISTS `department` (
  ...
  `department_name` VARCHAR(100) NULL DEFAULT 'Production',
  `fk_branch_id_department` INT(11) NULL DEFAULT '1',
   `created_at` TIMESTAMP,
  `update_at` TIMESTAMP,
  PRIMARY KEY (`department_id`),
  CONSTRAINT `fk_branch_id_department`
    FOREIGN KEY (`fk_branch_id_department`)
